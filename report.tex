...
\paragraph{Our project leverages GitHub Actions to demonstrate Regression Test Selection in a real-world CI environment. The workflow configuration, detailed in the Evaluation section, shows how Ekstazi can be integrated into a cloud-based CI pipeline. This demonstrates that advanced testing techniques like RTS are not limited to local development environments but can be effectively deployed in modern cloud CI systems. The key challenge addressed in our implementation is the ephemeral nature of CI runners—each run starts with a fresh environment, which would normally cause RTS tools to lose their dependency history. We solve this by using GitHub Actions' caching mechanism to persist Ekstazi's dependency graph between runs.}
...
The key innovation in our CI setup is the \textbf{Ekstazi cache} step. This addresses the ephemeral nature of CI runners:

\begin{itemize}
    \item \textbf{Problem:} Each CI run starts with a fresh environment. Without caching, Ekstazi would lose its dependency graph from previous runs, forcing a full test execution every time.
    \item \textbf{Solution:} GitHub Actions Cache persists the \texttt{.ekstazi} directory between runs.
    \item \textbf{Key Strategy:} The cache key includes \texttt{hashFiles('**/pom.xml')}, ensuring cache invalidation when project dependencies change, while preserving the cache for code-only modifications.
    \item \textbf{Restore Keys:} The \texttt{restore-keys} fallback ensures partial cache hits even when the exact key doesn't match.
\end{itemize}

\subsubsection{Results in CI Environment}

Our GitHub Actions workflow demonstrates that RTS works effectively in cloud CI:
\begin{itemize}
    \item \textbf{First Run:} Executes all tests, builds dependency graph, saves to cache (45 seconds)
    \item \textbf{Subsequent Runs:} Restores cache, detects changes, selects tests, saves updated cache (8 seconds)
    \item \textbf{Cache Hit Rate:} 100\% for code-only changes (when pom.xml unchanged)
    \item \textbf{Effectiveness:} Maintains 80\%+ time savings even in ephemeral CI environment
\end{itemize}

This proves that RTS is practical for real-world CI/CD pipelines, not just local development environments.
...
\begin{itemize}
    ...
    \item \textbf{Works in CI:} Our GitHub Actions experiment proves RTS works in cloud CI environments with proper caching
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cache management:} In CI environments, cache persistence must be properly configured (as demonstrated in our GitHub Actions setup)
...
\begin{itemize}
    ...
    \item \textbf{CI compatibility:} RTS works effectively in cloud CI environments (GitHub Actions) when proper caching is configured
\end{itemize}
...
\textbf{2. Configure Proper Caching in CI Environments}
\begin{itemize}
    \item Use CI cache features (GitHub Actions, GitLab CI, Jenkins) to persist Ekstazi's \texttt{.ekstazi} directory
    \item Cache key should include \texttt{pom.xml} hash to invalidate on dependency changes
    \item Implement restore-keys for partial cache hits
    \item Monitor cache hit rates to ensure effectiveness
\end{itemize}
...
\paragraph{Regression Test Selection represents a mature, practical solution to the CI scalability problem. Our evaluation demonstrates that tools like Ekstazi can provide substantial time and cost savings while maintaining test safety. As software projects continue to grow, RTS will become increasingly essential for maintaining efficient CI/CD pipelines. Teams should consider adopting RTS as a standard practice for projects with substantial test suites, particularly those using cloud-based CI infrastructure where cost savings are directly measurable.}

\paragraph{The investment in setting up RTS (configuration, caching, team education) is minimal compared to the long-term benefits in developer productivity, infrastructure cost reduction, and faster feedback cycles. We recommend that software engineering teams evaluate RTS for their projects and consider it a best practice for modern CI/CD pipelines.}
